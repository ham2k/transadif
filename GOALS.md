...
Refer to this GOALS.md file, and in particular the Implementation Details section, any time you are wondering about how to approach a specific problem.
...
The tool should offer a `--debug` option to print the contents of the specified QSOs. Including details for each field such as the original data in characters and bytes, and the interpreted Unicode data. Use this function to help in debugging the tool.

The test runner should also present errors by printing the bytes that are different between the actual output and the expected output, along with nearby text to help identify the problem.
...
The test running tool should consider a test case failed if it runs for more than 10 seconds.

Never create special logic in the code designed to hack around a specific test case.
...
* `-d, --debug <qso1,qso2,...>`: Debug mode, print the contents of the specified QSOs
